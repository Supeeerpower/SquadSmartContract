...
contract Marketplace is ReentrancyGuard {
    ...
    event BuyEnglishAuction(
        address indexed buyer, address indexed contractAddress, uint256 indexed nftId, uint256 price, uint256 time
    );
    ...
    event NewEnglishAuctionListing(
        address indexed _nftContractAddress, uint256 indexed _nftId, uint256 indexed _initialPrice, uint256 _salePeriod
    );
    ...
    event NewOfferingSaleListing(
        address indexed nftContractAddress, uint256 indexed nftId, uint256 indexed initialPrice
    );
    event NewBidToEnglishAuction(uint256 indexed id, uint256 indexed sendingValue, address indexed currentWinner);
    event NewWithdrawFromEnglishAuction(uint256 indexed id, address indexed from, uint256 indexed amount);
    ...
    function listToEnglishAuction(
        ...
        require(checkOwnerOfNFT(_nftContractAddress, _nftId) == true, "Invalid token owner");
        uint256 id = englishAuctions.length;
        englishAuction memory newAuction = englishAuction(_initialPrice, _salePeriod, address(0), _initialPrice);
        englishAuctions.push(newAuction);
        listedNFT memory newListing =
            listedNFT(SaleType.ENGLISH_AUCTION, id, msg.sender, _nftContractAddress, _nftId, block.timestamp, 0, false);
        englishAuction_listedNumber[id] = listedNFTs.length;
        listedNFTs.push(newListing);
        emit NewEnglishAuctionListing(_nftContractAddress, _nftId, _initialPrice, _salePeriod);
    }
    ...
    function listToDutchAuction(
        ...
        require(checkOwnerOfNFT(_nftContractAddress, _nftId) == true, "Invalid token owner");
        require(_initialPrice > _reducingRate * (_salePeriod / 3600), "Invalid information for Dutch Auction!");
        uint256 id = dutchAuctions.length;
        dutchAuction memory newAuction = dutchAuction(_initialPrice, _reducingRate, _salePeriod);
        dutchAuctions.push(newAuction);
        listedNFT memory newListing =
            listedNFT(SaleType.DUTCH_AUCTION, id, msg.sender, _nftContractAddress, _nftId, block.timestamp, 0, false);
        dutchAuction_listedNumber[id] = listedNFTs.length;
        listedNFTs.push(newListing);
        emit NewDutchAuctionListing(_nftContractAddress, _nftId, _initialPrice, _reducingRate, _salePeriod);
    }
    ...
    /// @param _nftId TokenId of the NFT contract
    /// @param _initialPrice Initial price of the NFT
    function listToOfferingSale(address _nftContractAddress, uint256 _nftId, uint256 _initialPrice) external {
        require(checkOwnerOfNFT(_nftContractAddress, _nftId) == true, "Invalid token owner");
        uint256 id = offeringSales.length;
        offeringSale memory newSale = offeringSale(_initialPrice, 0);
        offeringSales.push(newSale);
        listedNFT memory newListing =
            listedNFT(SaleType.OFFERING_SALE, id, msg.sender, _nftContractAddress, _nftId, block.timestamp, 0, false);
    ...
    function checkOwnerOfNFT(address _nftContractAddress, uint256 _nftId) public view returns (bool) {
    ...
    function makeBidToEnglishAuction(uint256 _id, uint256 _sendingValue) external {
        require(englishAuctions.length > _id && _id >= 0, "Not listed in the english auction list.");
        uint256 listedId = englishAuction_listedNumber[_id];
        require(cancelListingState[listedId] == false, "Listing Cancelled.");
        uint256 expectEndTime = listedNFTs[listedId].startTime + englishAuctions[_id].salePeriod;
    ...
    function endEnglishAuction(address _nftContractAddress, uint256 _nftId) external {
        ...
        for (uint256 i = 0; i < englishAuctions.length;) {
            ...
            if (listedNFTs[tmp_Id].nftContractAddress == _nftContractAddress && listedNFTs[tmp_Id].nftId == _nftId) {
                id = i;
                flg = true;
                break;
            }
        ...
        require(englishAuctions.length > id && id >= 0, "Not listed in the english auction list.");
        ...
        require(price > initialPrice, "No bidder!");
        uint256 loyaltyFee;
    ...
    /// @param _nftContractAddress Address of the NFT contract
    /// @param _nftId TokenId of the NFT contract
    function recordRevenue(address _seller, uint256 _price, address _nftContractAddress, uint256 _nftId) private {
    ...
    function buyDutchAuction(uint256 _id, uint256 _sendingValue) external {
        require(dutchAuctions.length > _id && _id >= 0, "Not listed in the dutch auction list.");
        uint256 listedId = dutchAuction_listedNumber[_id];
        require(cancelListingState[listedId] == false, "Listing Cancelled.");
        require(listedNFTs[listedId].endState == false, "Already sold out.");
        uint256 expectEndTime = listedNFTs[listedId].startTime + dutchAuctions[_id].salePeriod;
        require(expectEndTime > block.timestamp, "Auction ended!");
    ...
    function makeBidToOfferingSale(uint256 _id, uint256 _sendingValue) external {
        require(offeringSales.length > _id && _id >= 0, "Not listed in the offering sale list.");
        ...
        ICreatorGroup(currentOwner).submitOfferingSaleTransaction(_id, nftId, msg.sender, _sendingValue);
    ...
    function endOfferingSale(uint256 _id, address _buyer) external {
        ...
        emit BuyOfferingSale(_buyer, contractAddress, nftId, price, block.timestamp);
    }
        ...
        require(
            !(
                listedNFTs[id]._saleType == SaleType.ENGLISH_AUCTION
                    && englishAuctions[listId].currentPrice != englishAuctions[listId].initialPrice
            ),
            "Already english auction started!"
        );
        require(
            !(listedNFTs[id]._saleType == SaleType.OFFERING_SALE && offeringSales[listId].bidNumber != 0),
    ...
    function addBalanceOfUser(
        ...
        for (uint256 i = 0; i < listedNFTs.length;) {
            if (
                listedNFTs[i].nftContractAddress == _nftContractAddress && listedNFTs[i].nftId == _nftId
                    && listedNFTs[i].currentOwner == msg.sender && listedNFTs[i].endState == true
            ) {
                flag = true;
                break;
            }
    ...
    /// @dev Only callable by the owner
    function setPercentForLoyaltyFee(uint256 _percentForLoyaltyFee) external onlyOwner {
    ...
    function withdraw() external nonReentrant {
        ...
        uint256 amount = balanceOfDevelopmentTeam;
        balanceOfDevelopmentTeam = 0;
        if (amount != 0) SafeERC20.safeTransfer(USDC_token, msg.sender, amount);
    ...
    function withdrawFromSeller() external nonReentrant {
        ...
        uint256 amount = balanceOfSeller[msg.sender];
    ...
    function getDutchAuctionPrice(uint256 id) public view returns (uint256) {
        ...
        uint256 price = dutchAuctions[id].initialPrice
            - ((block.timestamp - listedNFTs[listedId].startTime) / duration) * dutchAuctions[id].reducingRate;
        return price;
    }
